...
      <img src="https://public.karat.io/content/front-end-coding-exercises/p2-1b-article-filter.png" height="30%" />
...
filterInput.oninput = (e) => {
    ...
    const articles = document.querySelectorAll("#articles li"); 
        ...
        article.dataset.original=article.textContent
        const words = article.textContent;
        const original = article.dataset.original
     if (!value) {
         // make the article visible here
        article.style.display=''
        article.textContent=words
        return 
    }
     ...
     if (words.includes(value)) {
        ...
        const highlighted = words.replace(regex,`<span class="bg-red">$1</span}`)
